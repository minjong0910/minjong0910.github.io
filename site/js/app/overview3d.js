...
function addLevelContents(g, lv, detail){
  ...
  if(lv==='B1'){
    ...
    /* B1도 다른 층처럼 정수기·휴지통을 표시한다 — B1은 화장실 박스가
    ...
    (function(){
      ...
      /* 상세보기(2D 층별 지도)에서 정수기·휴지통이 너무 작아 잘 안 보였다 —
    ...
    if(detail){
      ...
      if(lv===1){
        /* 1층만: 직접 걸어보기 3D에서는 정수기·휴지통이 화장실 칸 안이 아니라
...
function buildRouteA(){
  ...
  /* v149: 예전엔 목적지가 같은 층이어도 무조건 엘리베이터를 들렀다 →
...
function highlight3D(){
  ...
  floorGroups.forEach(function(g,i){
    ...
    g.traverse(function(o){
      ...
      /* 글씨(EV·계단 등)는 이제 모든 층에서 항상 보이게 한다 — 어느 층에서든
...
function animate(){
  ...
  if(fpCeil){
    ...
    /* 버그 수정: 이 공용 천장(fpCeil)은 어두운 남색 판이고 높이가
    ...
    /* 버그 수정: 이 공용 천장판은 건물 전체(24 x 105m)를 덮는 짙은
  ...
  if(fpCorrG){
    ...
    /* 버그 수정: 재생(경로 미리보기)은 '출발 층 + 목적지 층' 두 층을
  ...
  /* 실사형 개선(3차) : 평소(재생 중이 아닌) 1인칭 걷기의 기본 화각이 BASE_FOV(45,
     세로 화면 기준 가로 시야각 22도 남짓)라 망원렌즈처럼 답답해 보였다(보행자 시야에 맞춘 55~60도 대). 옥상·재생 중 화면에서 이미 어안 왜곡 없이
  ...
  if(fpFloorNow==='R') wantFov = 68;   // 답답하지 않은 원근감(58~63도보다 살짝 더 넓게, 어안 왜곡 없는 상한선 안쪽)
  if(rideBlend > 0){
