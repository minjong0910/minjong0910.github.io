...
/* 버그 수정: B1↔1F 한 도막 계단(straightUp)은 반 층이 아니라 전체 층
...
/* B1↔1F 한 도막 직선 계단은 한 층 전체(SP=6.6m)를 다른 층의 반 층짜리
...
function fpMakeStairwell(lv, zc, ko, em, roofH, owMul){
  ...
  /* B1↔1F 구간은 실사진(1번·4번 사진)처럼 계단 앞뒤로 여유 있는 로비 공간이
     ...
     보인다. 이 구간(B1의 오르는 쪽, 1F의 후문 로비 쪽) 양쪽 모두
  ...
  /* 1층 중앙계단은 폭을 1.4배 넓혔는데, 이 계단은 물리적으로
  ...
  /* B1↔1F 구간은 실제로 계단참 U턴 없이 한 방향으로 쭉 이어지는 단일 계단이다
     (사진 반영). 이 계단실 매스는 B1 쪽 그룹(lv==='B1', upLv===1)이
  ...
  /* 실제 사진 기준 마감 : 중앙계단 = 베이지 석재 타일 벽, 비상계단 = 크림색 페인트 벽.
     ...
     그대로 써서 유난히 밝아 보였다(옥상에서만 중앙계단과 비슷한
     중간 톤으로 낮춘다 — 다른 층의 크림색 마감은 그대로 둔다). */
  /* v124: 비상계단 옥상 그룹만 다른 회색빛(0xB2AC9C)을 쓰고 있어, 5층 그룹 벽과
  ...
  /* 구조 정정: 사용자가 실제 건물 기준을 다시 확인해 주었다 —
  ...
  /* 버그 수정: 이 계단실 문턱 바닥은 지금까지 공용 텍스처
  ...
  /* 버그 수정: fpMkTex는 오버레이용(depthWrite:false)이라, 이
  ...
  if(!em){          // 중앙계단 문턱의 노란 점자블록 — 다른 층은 문턱 폭(0.42m)만,
    ...
    if(lobby1F){
      /* 1층 : 밋밋한 노란 판 대신 실제 돌기(도트)가 그려진 점자 유도블록으로.
         ...
         유도블록을 두지 않는다 — 문턱을 가로지르는 유도 띠만 남긴다. */
  ...
  /* 1층→지하 후문 곁가지 : 직접 렌더링해서 확인해 보니, 이 구멍-옆 벽 조각이
     ...
     허공에 뜬 것처럼 겹쳐 보이는 게 확인됐다(원상복구). 벽 구멍을
  ...
  /* v104: 옥상 내려가는 계단과 -Z 벽 사이에 남아 있던 약 19cm 틈(그 틈으로
  ...
  [-1,1].forEach(function(sn){
    if(roofOpen){
      ...
      if(sn>0){
        /* 버그 수정, 근본 원인: 무릎 높이(1.05m)까지만 막고 그
  ...
  if(!roofOpen){
    if(!lobby1F){   // 1층 중앙계단은 fpMake1FStairHall이 유리 후문이 뚫린 뒷벽을 대신 세운다
      /* 버그 수정: 옥상 계단참 벽과 똑같은 원인 — 이 벽도 실사용
      ...
      /* 버그 수정: 이 뒷벽은 스침 각도에서 벽 밖이 보이지 않도록
    ...
    /* 버그 수정: 위층(upLv)이 있는 경우에도 이 천장판을 무조건
    ...
    /* 버그 수정: 위층이 있는 계단실은 천장을 아예 안 만들었는데,
    ...
    /* 버그 수정: 이 뚜껑을 yT(=중간 계단참보다 겨우 0.45m 위)에
    ...
    g.add(flC);
    /* 버그 수정: 계단실 입구 쪽(x0)은 복도 벽이 대신 막아 주는데,
    ...
    if(dHb>0.05){
      ...
      /* 버그 수정: 위 대응으로 이 벽을 세로로도 6m나 키워 놨는데
      ...
      /* 버그 수정, 세 번째 지점: 계단실 입구 쪽(x0=2.1)과 옥탑방
  ...
  /* 버그 수정: B1↔1층 직선 계단(straightUp)은 yT가 SP+0.45라
  ...
  /* 버그 수정: 이 '매립등 + 하우징 테두리'는 계단실 천장(yT)에 붙는
  ...
  if(lampOnCeil){
    /* v95: B1 직선계단의 이 등·하우징은 1층 바닥 밑면(y=SP)에 붙어 있는데,
  ...
  /* 이 조명은 계단실 "윗쪽"(yT 근처)에만 있어서, B1→1F 구간처럼
  ...
  /* v101: 5층↔옥상 계단참에도 다른 층처럼 -Z 옆벽 철문. 옥상 그룹(lv='R')이
  ...
  /* v125: 비상계단 1~5층 계단참(1↔2, 2↔3, 3↔4, 4↔5) 뒷벽에도 창문 2개 — 계단실이
  ...
  /* v128: 비상계단 5층↔옥상 계단참에도 같은 작은 창 2개 — 중앙계단과 같이 옥상 그룹이
  ...
  for(var i=0;i<N;i++){
    if(upLv){
      ...
      /* 디딤판(밝은 베이지)과 챌판(앞면)이 거의 같은 색이라, 위에서
  ...
  if(upLv && !(lv==='B1' && upLv===1)){
    ...
    /* 버그 수정: 1층에서만 착지참을 '올라가는 단 폭(fw+0.6)'으로
    ...
    var lndW = hw*2-0.08;
    var lndZ = zc;
    /* v113: 5층↔옥상 계단참은 옥상 그룹의 -Z 옆벽이 옥탑방 벽면(zc-hw-ROOF_WALL_IN)에
  ...
  function stairRail(zEdge, y0r, y1r, xStart, xEnd){
    ...
    var np=1;   // 단순화: 촘촘한 세로 창살 대신 양끝 기둥만 남긴다(가로 파이프가 주가 되게)
    for(var i=0;i<=np;i++){
      var t=i/np;
  ...
  if(upLv && !straightUp){
    ...
    /* 버그 수정: B1↔1F 직선 계단(straightUp)은 위 flight()에서
  ...
  /* 1층→지하 후문 + U턴 착지참 : 직접 렌더링해 확인해 보니 계단실 벽 구멍이
     ...
     되돌린다. 관련 함수 정의(fpMakeBackDoorBranch 등)는 남겨 둔다. */
  ...
  if(roofOpen || (lv==='R' && em)){
    /* 옥상(lv==='R')은 upLv가 없어 '오르는 단'이 애초에 없다 — 그런데도 이전
       ...
       이 칸막이/난간을 아예 짓지 않는다(정면 완전히 개방).
       버그 수정: 이 조건은 원래 중앙계단(roofOpen, em=false)만
    ...
    /* 비상계단은 사진처럼 가운데가 허리 높이 난간이라 입구에서 계단 전체가
       ...
       난간만 둥둥 떠 있는 것처럼 보였다(upLv 있을 때만 짓는다). */
    var midH = 1.05;
    ...
    /* 삭제: B1↔1층 직선 계단은 오르내리는 단이 한 열뿐이라 이
  ...
  function flight(sgn, zSide, dirUp){
    ...
    /* 예전엔 이 파라펫에 강한 emissive 틴트(0x33302A)를 줘서 MeshPhongMaterial로
       ...
       흰색 삼각형처럼 보이는 버그가 있었다(실사용 스크린샷에서 확인된
  ...
  // 개구부 테두리 + 안내 현판
  /* 삭제: 계단실 개구부 양옆에 세워 두던 가느다란 테두리 띠
  ...
  /* 실제 사진(중앙계단 앞)을 보면 중앙계단 개구부도 비상계단과
  ...
  if(lv!=='R' && lv!=='B1'){
    /* 비상계단 입구 : 회색 문틀 + 한쪽으로 살짝 열린 베이지색 방화문 + 초록 비상구 표지판
       ...
       문(fpMakeHeadhouse)이 이미 있으므로 여기서는 짓지 않는다. */
    /* 다듬기: 문틀 상자가 복도 쪽으로 0.30m나 튀어나와 있어서,
...
var FP_B1_DOOR_W = 1.9;      // 크리에이티브 존 출입문 폭 — 옆벽을 더 넓히기 위해 재조정(2.2→1.9, 옆벽 0.7m→0.85m씩)
...
/* 계단을 내려오면 바로 보이는 이 벽감이 너무 얕아(1.55m) 문 두 짝이
...
/* 1구간·계단참·2구간을 오갈 때 "계단을 밟으며 오르는" 바운스
...
/* 나가는 문(이중문)의 문 폭·높이 — 존 북쪽 끝벽(zback)에 이 문만큼 구멍을 뚫는
   ...
   (예전엔 zback이 통짜 한 장이라 문 유리 바로 뒤를 크림색 벽이
...
/* 계단실 폭 — 계단·계단참·양옆 벽돌벽·걸어다닐 수 있는 범위가 전부 이 값 하나를
   공유한다. 12.0m는 문(3.5m) 양옆에 벽돌이 4.25m씩이나 남아 너무
   휑해 보였다 → 8.0m로 줄여 양옆을 2.25m씩으로 좁힌다. */
var FP_B1_EXIT_SW   = 6.5;   // 8.0 → 6.5m (문 양옆 벽돌이 1.5m씩)
var FP_B1_EXIT_LAND_D = 3.2;             // 계단참(꺾이는 자리) 깊이 — 뒷벽이 너무 가까워 보여서 1.8→3.2로 더 뒤로 민다
